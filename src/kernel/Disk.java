...
public class Disk implements Runnable {
    ...
    public Disk(int size) {
        ...
        try {
            ...
            out.println("Creating new partion");
            this.format();
        } catch (Exception e) {
            e.printStackTrace();
            exit(1);
...
//        }
    ...
    public void flush() {
        try {
            out.println("Saving contents to partion file...");
            FileOutputStream os = new FileOutputStream("DISK");
            os.write(data);
            os.close();
            out.printf("%d read operation(s) and %d write operation(s) performed\n", readCount, writeCount);
        } catch(Exception e) {
            e.printStackTrace();
            exit(1);
        }
    ...
    public synchronized void beginRead(int blockNumber, byte[] buffer) {
        ...
        if (busy) {
            throw new DiskException("Disk read Was unable to process request " + " while the disk was still busy.");
        }
    ...
    public synchronized void beginWrite(int blockNumber, byte[] buffer) {
        ...
        if (busy) {
            throw new DiskException("unable to process disk write request " + " while the disk was still busy.");
        }
